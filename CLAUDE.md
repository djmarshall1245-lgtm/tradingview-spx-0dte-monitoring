...
which is why an in-session scan looks clean. **Disabled 2026-06-15. Do NOT
re-enable them on a schedule** — run the briefing manually at the desk instead.
...
Disable persistently (Mac terminal, NOT the web session):
    launchctl unload -w ~/Library/LaunchAgents/com.andrereynolds.spx.briefing.plist
    launchctl unload -w ~/Library/LaunchAgents/com.andrereynolds.lotteryscanner.plist
Verify gone: `launchctl list | grep -E 'briefing|lotteryscanner'` (no output = off).
Belt-and-suspenders: rename the plists (append `.disabled`) so nothing can load
them. The morning brief is still available ON DEMAND — trigger it yourself with
the briefing template line above when you sit down.
